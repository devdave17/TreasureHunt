...
import { formatCountdownLabel, formatDateTimeLabel } from "../utils/questTiming.js";
...
const QuestScreen = ({
  ...
  const userInitial = useMemo(() => {
    return String(userDetails?.name || "?").trim().charAt(0).toUpperCase() || "?";
  }, [userDetails]);

  const lockStartAtMs = Number(questLockNotice?.startAtMs) || 0;
  const lockSecondsLeft = Math.max(0, Math.ceil((lockStartAtMs - lockNowMs) / 1000));
      ...
      <div className="panel anim-reveal quest-dashboard-panel">
        <div className="quest-topbar">
          <div className="profile-dropdown-wrapper" ref={dropdownRef}>
            <button className="account-chip" onClick={() => setShowProfileModal((prev) => !prev)}>
            ...
            {showProfileModal && (
              <div className="profile-dropdown anim-dropdown-open">
                <div className="profile-dropdown-header">
                  <div className="profile-avatar profile-avatar-dropdown">{userInitial}</div>
                  <div className="profile-dropdown-info">
                    <div className="profile-dropdown-name">{userDetails?.name || "Unknown"}</div>
        ...
        <div className="panel-sub">
          Welcome, <strong style={{ color: "var(--gold)" }}>{userDetails?.name}</strong> ({userDetails?.email}).
          Please select a quest card below to continue.
        </div>

        <div className="quest-grid">
          {loadingQuests && <div className="quest-empty">Loading quests...</div>}
          ...
          {!loadingQuests &&
            quests.map((quest) => {
              ...
              return (
                <div key={quest.id} className={`quest-card ${isSelected ? "active" : ""}`}>
                  ...
                  <div className="quest-card-desc">
                    {quest.description || "Complete this quest to unlock all checkpoints and reach the treasure."}
                  </div>
                  ...
                  <div className="quest-card-actions">
                    ...
                    <button
                      ...
                      disabled={isLaunching}
                    >
                      {isLaunching && isSelected ? "⏳ Starting..." : "Start Quest"}
                    </button>
                  </div>
                </div>
              );
      ...
      {questLockNotice && (
        <div className="modal-overlay" onClick={onCloseQuestLockNotice}>
          <div className="profile-modal" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="profile-name" style={{ fontSize: "1.1rem", marginBottom: "0.45rem" }}>
            ...
            <div className="profile-detail-list" style={{ marginTop: 0 }}>
              ...
              <div className="profile-detail-row">
                ...
                <span className="profile-detail-value">{formatCountdownLabel(lockSecondsLeft)}</span>
              </div>
              <div className="profile-detail-row">
                <span className="profile-detail-label">Live At</span>
                <span className="profile-detail-value">{formatDateTimeLabel(lockStartAtMs)}</span>
              </div>
            </div>
            <div className="btn-group" style={{ justifyContent: "flex-end" }}>
              <button className="btn-secondary" onClick={onCloseQuestLockNotice}>
                OK
              </button>
            </div>
          </div>
