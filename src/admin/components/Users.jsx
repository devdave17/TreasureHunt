...
function Users({ authToken, role }) {
  ...
  const handleLevelUpdate = async (userId, newLevel, currentLevel) => {
  ...
  const handleDeleteAllUsers = async () => {
    const confirmed = window.confirm("Delete ALL users? This cannot be undone.")
    if (!confirmed) {
      return
    }
            ...
            <tbody>
              {sortedUsers.length === 0 ? (
                ...
                sortedUsers.map((user, idx) => {
                  ...
                  const isBusy = blockingUserId === user.id || updatingUserId === user.id || deletingUserId === user.id
